...
(defun parse-module (module &key build-module-p)
  "Takes a module declaration form and returns a module object representing that module.  Inherits licence, author, and maintainer slots from the build-module, if not specifically overwritten"
...
(defun resolve-module (module-path &key (parent-module nil parent-module-supplied-p) build-module-p)
  "Takes a filepath to a lisp file, and returns the module object represented by the module specifer at the top of that lisp file.  If the argument parent-module is supplied, then the new module will be given a fullname relative to the fullname of parent-module (if the fullname isn't already specified)"
...
(defun get-module (name &optional (build-module *build-module*))
  ...
  (or (gethash (strcat (fullname build-module) "/" name) *module-map*)
      ...
      (error 'simple-error :format-control "The module with name \"~a\" cannot be found" :format-arguments name)))
