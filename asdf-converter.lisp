...
(defun module-string (module)
  (with-output-to-string (out)
    ...
    (if (fullname module)
      (format out "~%~T:fullname ~s" (fullname module)))
    (if (author module)
      (format out "~%~T:author ~s" (author module)))
    (if (maintainer module)
      (format out "~%~T:maintainer ~s" (maintainer module)))
    (if (version module)
      (format out "~%~T:version ~s" (version module)))
    (if (description module)
      (format out "~%~T:description ~s" (description module)))
    ...
    (if (load-depends-on module)
      (format out "~%~T:load-depends-on (~%~{       ~s~^~%~})" (load-depends-on module)))
    (if (build-depends-on module)
      (format out "~%~T:build-depends-on (~%~{       ~s~^~%~})" (build-depends-on module)))
    (format out ")~%")))
