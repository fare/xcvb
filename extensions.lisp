...
(defparameter *extension-functions-map* (make-hash-table)
  "Map of keywords that may appear in the extension-forms part of a module declaration
to functions")
...
(defun handle-extension-forms (module)
  "This handles the extension forms from the module declaration.  These forms can do
things such as (but not limited to) change slots in the module, specify system-wide
dependencies, or extend xcvb itself.  If an extension function needs to have access to
the module object that it is declared in, then when writing the extension form in the
module declaration of a file, use the keyword :this-module to represent the current
module. The extension function will be passed the current module instead of any
arguments that are :this-module."
...
(defextension add-dependencies-to-module :add (module dep-type deps)
  "Extension form that can be put in a module declaration to add additional dependencies
to that module"
...
(defextension set-module-slot :set (module slot-name value)
  "Extension form that can be put in a module declaration to overwrite the dependencies
of that module"
...
(defextension load-into-xcvb :xcvb-requires (deps)
  "Extension form that can be put in a module declaration so that xcvb will load the
given file into itself.  This provides a good facility for users to load their own
extensions into xcvb"
  (load-systems deps))
