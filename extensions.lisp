...
(defextension remove-dependency-from-module :remove (module dep-type value)
  ...
  #|(when (or (eql dep-type :compile-depends-on) (eql dep-type :compile-and-load-depends-on))
    (setf (slot-value module 'compile-depends-on) 
          (remove value (slot-value module 'compile-depends-on) :test #'equal)))
  (when (or (eql dep-type :load-depends-on) (eql dep-type :compile-and-load-depends-on))
    (setf (slot-value module 'load-depends-on) 
          (remove value (slot-value module 'load-depends-on) :test #'equal)))
  (when (not (or (eql dep-type :compile-depends-on) (eql dep-type :load-depends-on) (eql dep-type :compile-and-load-depends-on)))
    (error "Invalid property for :remove operation, must be one of (:compile-depends-on :load-depends-on :compile-and-load-depends-on")))|#
  (ecase dep-type
    (:compile-depends-on 
       (setf (slot-value module 'compile-depends-on) 
             (remove value (slot-value module 'compile-depends-on) :test #'equal)))
